$(document).ready(function () {
    ...
    jQuery('#comment_btn').click(function (e) {

        e.preventDefault();
        var form_comment = new FormData($('#comment_form')[0]);
        jQuery.ajax({
            url: "/comment",
            data: form_comment,
            type: "POST",
            ...
            datatype: "json",
            success: function (data) {
                // jquery('.comment-sec-area').html(data)
                var count = parseInt($('#comments_count').text().split(' ')[0]);
                count = ++count;
